...
- Check to see if the serialization can be compressed. For example, [`joblib`](https://joblib.readthedocs.io/en/latest/) has a `compress` argument.
- Uplaod all the source code and provide instructions for the evaluator to train the model locally. If training are very time or resource consuming, provide a link to download the model for the evaluator's conven 
