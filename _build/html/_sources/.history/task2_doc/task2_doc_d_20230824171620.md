...
For regression, a metric measuring the error of the model should be used. Unlike classification models, a regression estimates continuous values and regression metrics take different approaches to quantifying how close these estimates are to actual values. Common metrics include mean square error (MSE), mean absolute error (MAE), and mean absolute percentage error (MAPE).
